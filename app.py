...
<style>
    ...
    /* ─── Tab styling: dark text, visible always ─── */
    ...
    /* ─── Expander styling: always readable ─── */
    ...
    /* Also target newer Streamlit expander classes */
   ...
   /* ─── Upload area ─── */
    ...
    .kpi-value {
        font-size: 1.4rem; font-weight: 700; color: #1B2A4A !important;
        margin: 0;
    }
    .kpi-label {
        font-size: 0.75rem; color: #666 !important; text-transform: uppercase;
        letter-spacing: 0.5px; margin: 0.3rem 0 0 0;
    }

    /* ─── Section headers ─── */
    .section-header {
        font-size: 1.3rem; font-weight: 700; color: #1B2A4A !important;
        border-bottom: 3px solid #3B7DD8;
        padding-bottom: 0.5rem; margin: 1.5rem 0 1rem 0;
    }

    .subsection-header {
        font-size: 1rem; font-weight: 600; color: #2C3E6B !important;
    ...
    .swot-box {
        ...
        min-height: 150px; color: white !important;
    }
    .swot-box * { color: white !important; }
    ...
    .qual-card h4 {
        margin: 0 0 0.3rem 0; font-size: 0.85rem; color: #1B2A4A !important; font-weight: 700;
    }
    .qual-card .score {
        font-size: 1.5rem; font-weight: 700; color: #3B7DD8 !important; margin: 0;
    }
    .qual-card .detail {
        font-size: 0.78rem; color: #666 !important; margin: 0.2rem 0 0 0; line-height: 1.4;
    }
...

{
  ...
  "page_4_market_analysis": {
    ...
    "location_quality": {
      "walkability": {
        "score": "Rate 1-10 based on OM info about nearby retail, restaurants, services within walking distance",
        "description": "Describe the walkability of the area based on what the OM mentions about nearby amenities, retail, dining, grocery, etc."
      },
      "transit_access": {
        "score": "Rate 1-10 based on proximity to public transit, highways, and commute routes mentioned",
        "description": "Describe public transit and commute access based on OM mentions of bus lines, metro, highways, commute times"
      },
      "school_quality": {
        "score": "Rate 1-10 based on any school information mentioned in or inferable from the OM",
        "nearby_schools": ["List any schools mentioned or referenced in the OM with any ratings or details provided"],
        "description": "Assess school quality based on OM data. If the OM mentions school districts, ratings, or proximity to specific schools, note them. If not mentioned, state that and note it as a data gap."
      },
      "safety_and_crime": {
        "score": "Rate 1-10 based on any safety or crime information mentioned or inferable from the OM and neighborhood context",
        "description": "Assess safety based on OM language, neighborhood description, and any crime data referenced."
      },
      "lifestyle_amenities": {
        "score": "Rate 1-10 based on nearby parks, gyms, entertainment, dining, and recreation mentioned",
        "nearby_amenities": ["List all nearby amenities, parks, entertainment, gyms, shopping mentioned in the OM"],
        "description": "Describe the lifestyle appeal of the area based on OM mentions of parks, recreation, entertainment, dining, nightlife, shopping."
      },
      "overall_location_grade": "Give an overall letter grade (A+ through F) for the location based on your assessment of walkability, transit, schools, safety, and lifestyle amenities. Justify briefly."
    }
  },
...
- For the location_quality section, provide your best assessment based on all available information in the OM. Score each dimension 1-10 where 1 is very poor and 10 is excellent. If the OM does not provide enough data for a confident rating, give your best estimate based on the neighborhood context and note the limited data.
...
def render_qual_card(title, score, description):
    st.markdown(f"""
    <div class="qual-card">
        <h4>{title}</h4>
...
if st.session_state.extracted is None:
        ...
        if uploaded_file:
            with st.status("Analyzing your OM...", expanded=True) as status:
                ...
                try:
                    ...
                    st.error("Invalid API key. Check your ANTHROPIC_API_KEY in Streamlit Secrets.")
    ...
    with tab1:
        ...
        # Property details in styled box
        c1, c2 = st.columns(2)
        with c1:
            st.markdown(f"""<div class="prop-detail-box">
                <strong>Address:</strong> {prop.get('address', 'N/A')}, {prop.get('city', '')}, {prop.get('state', '')} {prop.get('zip', '')}<br>
                <strong>Year Built:</strong> {prop.get('year_built', 'N/A')} &nbsp;|&nbsp; <strong>Renovated:</strong> {prop.get('year_renovated', 'N/A')}<br>
                <strong>Type:</strong> {prop.get('property_type', 'N/A')} &nbsp;|&nbsp; <strong>Construction:</strong> {prop.get('construction_type', 'N/A')}
            </div>""", unsafe_allow_html=True)
        with c2:
            st.markdown(f"""<div class="prop-detail-box">
                <strong>Total Units:</strong> {prop.get('total_units', 'N/A')} &nbsp;|&nbsp; <strong>Total SF:</strong> {f"{prop.get('total_sf'):,.0f}" if prop.get('total_sf') else 'N/A'}<br>
                <strong>Buildings:</strong> {prop.get('number_of_buildings', 'N/A')} &nbsp;|&nbsp; <strong>Stories:</strong> {prop.get('number_of_stories', 'N/A')}<br>
                <strong>Parking:</strong> {prop.get('parking_spaces', 'N/A')} &nbsp;|&nbsp; <strong>Lot:</strong> {prop.get('lot_size_acres', 'N/A')} acres
            </div>""", unsafe_allow_html=True)
        ...
        # Key Features — NOT in expander, just a styled section
        ...
        # Transparency Report — NOT in expander
        ...
        quality_notes = transparency.get("data_quality_notes", "")
        if quality_notes:
            st.markdown(f"**Data Quality:** {quality_notes}")
        ...
        missing = transparency.get("missing_data_points", [])
        if missing:
    ...
    with tab2:
        ...
        # Assumptions — plain section
        st.markdown('<div class="subsection-header">📐 Scenario Assumptions</div>', unsafe_allow_html=True)
        ...
        # Loan Schedule — plain section, NOT in expander
        st.markdown('<div class="subsection-header">🏦 Annual Loan Summary</div>', unsafe_allow_html=True)
    ...
    with tab3:
        ...
        # Lease Abstraction — plain section
    ...
    with tab4:
        ...
        # ─── Location Quality Assessment (NEW) ───
        loc_quality = market.get("location_quality", {})
        if loc_quality:
            st.markdown('<div class="subsection-header">📍 Location Quality Assessment</div>', unsafe_allow_html=True)

            # Overall grade
            overall = loc_quality.get("overall_location_grade", "")
            if overall:
                st.markdown(f'<div class="prop-detail-box"><strong>Overall Location Grade:</strong> {overall}</div>', unsafe_allow_html=True)

            # Score cards
            lq1, lq2, lq3, lq4, lq5 = st.columns(5)

            walk = loc_quality.get("walkability", {})
            transit = loc_quality.get("transit_access", {})
            schools = loc_quality.get("school_quality", {})
            safety = loc_quality.get("safety_and_crime", {})
            lifestyle = loc_quality.get("lifestyle_amenities", {})

            with lq1:
                render_qual_card("🚶 Walkability", walk.get("score", "N/A"), walk.get("description", ""))
            with lq2:
                render_qual_card("🚌 Transit Access", transit.get("score", "N/A"), transit.get("description", ""))
            with lq3:
                render_qual_card("🎓 School Quality", schools.get("score", "N/A"), schools.get("description", ""))
            with lq4:
                render_qual_card("🛡️ Safety", safety.get("score", "N/A"), safety.get("description", ""))
            with lq5:
                render_qual_card("🎯 Lifestyle", lifestyle.get("score", "N/A"), lifestyle.get("description", ""))

            st.markdown("")

            # Nearby schools detail
            ...
            nearby_amenities = lifestyle.get("nearby_amenities", [])
            if nearby_amenities:
                st.markdown("**Nearby Amenities:**")
                am_cols = st.columns(3)
                third = len(nearby_amenities) // 3 + 1
                for i, col in enumerate(am_cols):
                    with col:
                        for a in nearby_amenities[i*third:(i+1)*third]:
                            st.markdown(f"📍 {a}")
    ...
    with tab5:
        ...
        # Investment Highlights — plain section
        highlights = bp.get("investment_highlights_as_stated", [])
        if highlights:
            st.markdown('<div class="subsection-header">💡 Investment Highlights (as stated by broker)</div>', unsafe_allow_html=True)
            for h in highlights:
                st.markdown(f"- {h}")
        ...
        # Credibility Assessment — plain section
        ...
        if cred:
            ...
            st.markdown(f'<div class="prop-detail-box">{cred}</div>', unsafe_allow_html=True)
